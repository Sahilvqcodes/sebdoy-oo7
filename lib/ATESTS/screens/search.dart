...
class _SearchState extends State<Search> {
  ...
  void initState() {
    ...
    isHome = true;
    isAllKey = true;
    isPollKey = messages == "false" ? true : true;
    isSearchAllKey = true;
    showMessages = false;
    showPolls = false;
    ...
    pollkeystore == ""
        ? getValueG().then(((value) =>
            getValueM().then((value) => initPollList(pollkeystore!))))
        : "";
  ...
  Future<void> setValueG(String valueg) async {
    ...
    setState(() {
      ...
      } else if (showPolls == true) {
        initPollList(pollkeystore!);
      }
    });
  }
  ...
  Future<void> getData() async {
    ...
    await (global == "true"
      ...
      for (int i = 0; i < querySnapshot.docs.length; i++) {
        ...
        // print("fetchedlist");
        // print(fetchedList);
        List keyTags = fetchedList;
        keyTags = keyTags.map((tags) => tags.toLowerCase()).toList();
        print("fetchedList");
        print(keyTags);
        var result = keyTags.toSet().toList();
        // print("result");
        // print(result);
  ...
  var AllPollList = [];
  var fetchedPollList = [];
  List trendingPollKey = [];
  List trendingPollkeyvalue = [];
  List PollList = [];

  var AllPollList1 = [];
  var fetchedPollList1 = [];
  List trendingPollKey1 = [];
  List trendingPollkeyvalue1 = [];
  List PollList1 = [];
  Future<void> getPollTagData() async {
    // Get docas from collection reference
    ...
    await (global == "true"
      ...
      AllPollList.clear();
      for (int i = 0; i < querySnapshot.docs.length; i++) {
        fetchedPollList = querySnapshot.docs[i]["tags"];
        print("fetchedlist");
        print(fetchedList);
        List keyTags = fetchedPollList;
        keyTags = keyTags.map((tags) => tags.toLowerCase()).toList();
        print("fetchedList");
        print(keyTags);
        var result = keyTags.toSet().toList();
      ...
      PollList.clear();
      Map listMap = myMap;
      myMap.forEach((k, v) => PollList.add(Customer(k, v)));
      PollList.sort((b, a) => a.tagValue.compareTo(b.tagValue));
      return AllPollList;
    });
  }
  ...
  Future<void> getsearchData(String text) async {
    await (global == "true"
      ...
      for (int i = 0; i < querySnapshot.docs.length; i++) {
        ...
        // print("fetchedlist");
        // print(fetchedList);
        List keyTags1 = fetchedList1;
        keyTags1 = keyTags1.map((tags) => tags.toLowerCase()).toList();
        // print("fetchedList");
        // print(keyTags1);
        var result = keyTags1.toSet().toList();
  ...
  List _searchPollResult1 = [];
  Future<void> getSearchPollData(String text) async {
    print("text");
    print(text);
    await (global == "true"
            ? FirebaseFirestore.instance.collection('polls')
      ...
      AllPollList1.clear();
      for (int i = 0; i < querySnapshot.docs.length; i++) {
        fetchedPollList1 = querySnapshot.docs[i]["tags"];
        // print("fetchedlist");
        // print(fetchedList);
        List keyTags2 = fetchedPollList1;
        keyTags2 = keyTags2.map((tags) => tags.toLowerCase()).toList();
        print("fetchedList");
        print(keyTags2);
        var result = keyTags2.toSet().toList();

        AllPollList1.addAll(result);
      }

      final myMap = Map();

      AllPollList1.forEach(
        (element) {
          if (!myMap.containsKey(element)) {
            myMap[element] = 1;
          } else {
      ...
      PollList1.clear();
      Map listMap = myMap;
      myMap.forEach((k, v) => PollList1.add(Customer(k, v)));
      PollList1.sort((b, a) => a.tagValue.compareTo(b.tagValue));

      _searchPollResult1 = PollList1.where((user) =>
          user.tagName.toLowerCase().startsWith(text.toLowerCase())).toList();
    });
  }
  ...
  initPollList(String keyPollTags) async {
    ...
    loadDataStreamPoll = (global == "true"
        ...
        // .orderBy("datePublished", descending: false)
        .where("tagsLowerCase", arrayContains: keyPollTags)
        .snapshots()
        .listen((event) {
      for (var change in event.docChanges) {
        switch (change.type) {
          ...
          body: isUser && searchController.text.isEmpty
              ...
              : isUser
                  ...
                  : messages == "false" &&
                      ...
                      : messages == "false" && searchController.text.isNotEmpty
                          ? Center(
                              child: Padding(
                                padding: const EdgeInsets.symmetric(
                                    horizontal: 12.0),
                                child: Container(
                                  ...
                                  child: Padding(
                                    padding: const EdgeInsets.symmetric(
                                        horizontal: 0.0, vertical: 4),
                                    child: FutureBuilder(
                                        future: getSearchPollData(
                                            searchController.text),
                                        builder:
                                            (BuildContext context, snapshot) {
                                          return _searchPollResult1.isNotEmpty
                                              ? ListView.builder(
                                                  itemCount:
                                                      _searchPollResult1.length,
                                                  itemBuilder: (context,
                                                          index) =>
                                                      Padding(
                                                        padding:
                          ...
                          : showPolls && isHome && messages == "false"
                              ? ListView.builder(
                                                                          ...
                                                                          (context, index) =>
                                                                              Padding(
                                                                                ...
                                                                                child: GestureDetector(
                                                                                  ...
                                                                                  child: NoRadioListTile<String>(
                                                                                    start: (index + 1).toString(),
                                                                                    center: _searchResult[index].tagName ?? "",
                                                                                    end: _searchResult[index].tagValue.toString(),
                                                                                  ),
                                                                                ),
                                                                              ))
                                                              : Center(
